...
def make_one_timestep(G, seed):
        ...
        for i, u in enumerate(np.random.permutation(G.nodes())):
            seed += i
            set_seed(seed)
            topk_edges = recommend_nodes(u, G)
            influencers = [(u,v) for _,v,_ in topk_edges]   
            number_of_edges_to_remove = min(len(influencers), len(list(G.successors(u))))
            edges_to_be_removed = rewiring_list(G, u, number_of_edges_to_remove)
            G.remove_edges_from(edges_to_be_removed) # deleting previously existing links
            G.add_edges_from(influencers)
            seed += 1
        return G
