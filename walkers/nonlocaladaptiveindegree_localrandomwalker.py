...
class NonLocalAdaptiveInDegreeLocalRandomWalkerBeepBoop(Walker):
    def __init__(self,graph,beta=0,workers=1,dimensions=64,walk_len=10,num_walks=200):
        print("beepbop Non Local Adaptive In Degree Walker with beta = {} & Local Random".format(beta))
    ...
    def _precompute_alpha(self):
        ...
        for uniquegroup in uniquegroups:
            u = self.avg_outdegree_to_grp(uniquegroup)
            v = self.avg_indegree_due_to_grp(uniquegroup)
            group2alpha[uniquegroup] = u*(1-v)
